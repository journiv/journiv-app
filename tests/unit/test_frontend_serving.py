...
def frontend_client(tmp_path: Path) -> TestClient:
    ...
    (react / "manifest.json").write_text("{}")
    (react / "pwa").mkdir()
    (react / "pwa" / "maskable-icon-192x192.png").write_bytes(b"\x89PNG")
...
def test_manifest_json_is_no_cache(frontend_client: TestClient):
    response = frontend_client.get("/manifest.json")
    assert response.status_code == 200
    assert response.headers["cache-control"].startswith("no-cache")


def test_pwa_icons_fall_through_to_the_normal_bounded_asset_cache(
    frontend_client: TestClient,
):
    """`/pwa/*` is not under `assets/`, so it is not immutable-cached, but it
    is content-stable (regenerated by name, not fingerprinted) so the bounded
    weekly cache is correct -- and the no-cache manifest means a changed icon
    set is discovered on the next manifest fetch regardless."""
    response = frontend_client.get("/pwa/maskable-icon-192x192.png")
    assert response.status_code == 200
