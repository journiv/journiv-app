...
async def reorder_moods(
    ...
    try:
        ...
        return None
    except (MoodNotFoundError, ValidationError) as exc:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(exc),
        ) from exc
...
async def get_mood_groups(
    ...
    try:
        return service.get_groups_for_user(current_user.id)
    except Exception as exc:
        log_error(exc, request_id=None, user_id=current_user.id)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def reorder_mood_groups(
    ...
    except (MoodGroupNotFoundError, ValueError, ValidationError) as exc:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(exc),
        ) from exc
