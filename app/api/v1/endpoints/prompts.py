...
from app.schemas.prompt import PromptPageResponse, PromptResponse
from app.schemas.prompt_analytics import PromptAnalyticsResponse
...
@router.get(
    "/",
    response_model=PromptPageResponse,
    responses={
...
async def get_system_prompts(
    ...
    max_minutes: Optional[int] = Query(None, ge=0),
    ...
    if (
        ...
        raise HTTPException(
            ...
            detail="min_minutes must be less than or equal to max_minutes",
        )
    prompt_service = PromptService(session)
    try:
        prompts = prompt_service.get_system_prompts(
            category=category,
        ...
        total = prompt_service.count_system_prompts(
            category=category,
            difficulty_level=difficulty_level,
            q=q,
            min_minutes=min_minutes,
        ...
        category_counts = prompt_service.count_system_prompts_by_category(
            difficulty_level=difficulty_level,
            q=q,
            min_minutes=min_minutes,
            max_minutes=max_minutes,
        )
        next_offset = offset + len(prompts)
        return PromptPageResponse(
            items=prompt_service.prompt_responses(prompts, current_user.id),
            total=total,
...
async def get_random_prompt(
    ...
    try:
        ...
        if not prompt:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="No prompts available"
            )
        return prompt_service.prompt_response(prompt, current_user.id)
...
async def search_prompts(
    ...
    try:
        ...
        return prompt_service.prompt_responses(prompts, current_user.id)
    except Exception as e:
        log_error(e, request_id="", user_email=current_user.email)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
